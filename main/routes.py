...
def additems():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Submitted!')
        return redirect(url_for('additems', idchoice=idchoice))
...
@app.route('/cae/')
def cae_home():
    return render_template('caehome.html')


@app.route('/setupdb')
def setupdb():
    db.engine.execute('CREATE TABLE items(itemID INTEGER PRIMARY KEY AUTOINCREMENT,\
                      itemName TEXT,\
